...
- Its signatures are deterministic.
...
7. Use the master certification key to create a positive self-certification signature on the master key, committing to `uid` and `creation`.
    ...
    If `ttl` is defined, use it to define the master key expiry time `creation + ttl`, and commit the self-certification signature to that expiry. This signature should also commit to the symmetric cipher algorithm preferences `AES256 AES192 AES128`, the hash algorithm preferences `SHA512 SHA384 SHA256 SHA224 SHA1`, and the compression algorithm preferences `ZLIB BZIP2 ZIP`.
...
8. Use the master certification key to create binding signatures on each subkey, committing to `creation`.
    ...
    Note that `signingBindSig` must also include [a cross-certification signature on the master key](https://gnupg.org/faq/subkey-cross-certify.html), nested in the _unhashed_ signature subpackets.
...
- In the self-certification signature:
...
- Also in the self-certification signature:
  ...
  - Signed subpackets should be hashed in the following order:
    - Issuer fingerprint
    - Signature creation time
    - Key flags
    - Expiry
    - Cipher algorithm preferences
...
- The encryption subkey should be configured with key derivation parameters set to SHA256 and AES128.
...
- The signing subkey certification signature must include [a cross-certification signature](https://gnupg.org/faq/subkey-cross-certify.html) subpacket which proves the subkey accepts ownership by the master key.
